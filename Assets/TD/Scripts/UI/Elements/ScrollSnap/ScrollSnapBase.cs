...
namespace UnityEngine.UI.Extensions
{
    public class ScrollSnapBase : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IScrollSnap, IPointerClickHandler
    {
        ...
        public bool IsNormalizedScroll;
        ...
        {
            if (IsNormalizedScroll)
            {
                var factor = panelDimensions.width / _screensContainer.rect.width;
                return _isVertical ?
                    (int)Math.Round((_scrollStartPosition - pos.y) / _childSize * factor) :
                    (int)Math.Round((_scrollStartPosition - pos.x) / _childSize * factor);
            } else
            {
                return _isVertical ?
        ...
        {
            _childPos = IsNormalizedScroll ? -_childSize * page * panelDimensions.width / _screensContainer.rect.width : -_childSize * page ;
            if (_isVertical)
            {
